#pragma once
    ...
    #include "../common/ver.txt" // for SEL_C
        ...
        namespace SEL {
            class Events {
            public:
                static int Init() noexcept { return SEL_Init(); }
                static int AddEvent(const char* name) noexcept { return SEL_AddEvent(name); }
                static int RemoveEvent(const char* name) noexcept { return SEL_RemoveEvent(name); }
                static int ClearEvents() noexcept { return SEL_ClearEvents(); }
                static int Bind(const char* name, void(*func)(va_list)) noexcept { return SEL_Bind(name, func); }
                static int Unbind(const char* name, void(*func)(va_list)) noexcept { return SEL_Unbind(name, func); }
                static int TriggerV(const char* name, va_list args) noexcept { return SEL_TriggerV(name, args); }
                static int Trigger(const char* name, ...) noexcept {
                    va_list args;
                    va_start(args, name);
                ...
                static int Free() noexcept { return SEL_Free(); }
            };
        }
    #endif
#endif